 ...
 * AUTH PLAIN, UTF-8 in Betreff und Text. Kann nicht: Anhänge, HTML,
 ...
 * @param {string} o.text
 * @param {?{name:string,typ:string,inhalt:string}} [o.anhang]
...
function baueNachricht({ von, an, betreff, text, anhang }) {
  const gemeinsam = [
    `From: ${von}`,
    `To: ${an.join(", ")}`,
