...
export function smsText(fenster, maxZeichen = 160) {
  ...
  const zeilen = fenster.map((f) => {
    const vorn = einTag ? "" : labelVon(f.datum) + " ";
    return `${vorn}${kurz(f)} ${uhr(f.von)}-${uhr(f.bis)}h ${f.wind}kn ${f.richtung}`;
  });

  let text = kopf;
  let gezeigt = 0;
  for (const zeile of zeilen) {
    const rest = zeilen.length - gezeigt - 1;
    const anhang = rest > 0 ? `\n+${rest} weitere` : "";
    if ((text + "\n" + zeile + anhang).length > maxZeichen) break;
    text += "\n" + zeile;
    gezeigt++;
  }
  ...
  const rest = zeilen.length - gezeigt;
  return rest > 0 ? `${text}\n+${rest} weitere` : text;
}
...
export function mailInhalt(fenster, dashboardUrl, kalenderUrl = "") {
  ...
  const fuss =
    ...
    (kalenderUrl
      ? `\n\nDie angehängte Datei trägt die Fenster als Termine ein.\n` +
...
export async function melde(fenster, optionen = {}) {
  ...
  await versende(kanal, text, mail, env, fetchImpl, optionen.ics);
...
async function versende(kanal, text, mail, env, fetchImpl, ics = null) {
  // SMS trägt keinen Anhang - dort steht der Link zum Kalender im Dashboard.
  switch (kanal) {
    case "smtp":
      return sendeSmtp(mail, env, ics);
    case "brevo":
      return sendeBrevo(mail, env, fetchImpl, ics);
    case "aspsms":
      return sendeAspsms(text, env, fetchImpl);
    case "twilio":
      return sendeTwilio(text, env, fetchImpl);
    case "telegram":
      return sendeTelegram(text, env, fetchImpl, ics);
    default:
 ...
 */
async function sendeSmtp(mail, env, ics = null) {
  await sendeMail({
    host: env.SMTP_HOST,
    ...
    betreff: mail.betreff,
    text: mail.text,
    anhang: ics ? { name: "windfenster.ics", typ: "text/calendar; charset=utf-8", inhalt: ics } : null,
  });
}

/** E-Mail über die Brevo-HTTP-API. Gratis bis 300 Nachrichten am Tag. */
async function sendeBrevo(mail, env, fetchImpl, ics = null) {
  const res = await fetchImpl("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      ...(ics
        ? {
            attachment: [
              { content: Buffer.from(ics, "utf8").toString("base64"), name: "windfenster.ics" },
            ],
          }
        : {}),
    }),
 ...
 */
async function sendeTelegram(text, env, fetchImpl, ics = null) {
  ...
  for (const chatId of ziele) {
    try {
      ...
      if (ics) {
        try {
          await sendeTelegramDatei(chatId, ics, env, fetchImpl);
        } catch (e) {
          console.warn(`Kalenderdatei an ${chatId} nicht zugestellt: ${e.message}`);
        }
      }
    } catch (e) {
...
/** Kalenderdatei als Dokument. Outlook öffnet sie direkt als Termin. */
async function sendeTelegramDatei(chatId, ics, env, fetchImpl) {
